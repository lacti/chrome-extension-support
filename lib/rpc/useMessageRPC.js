...
function useMessageRPC(_a) {
    ...
    function listen() {
        ...
        chrome.runtime.onMessage.addListener(function (request) { return __awaiter(_this, void 0, void 0, function () {
            var callId, functionName, payload, result, error_1, _a, result, error;
            return __generator(this, function (_b) {
                switch (_b.label) {
                    ...
                    case 2:
                        ...
                        chrome.runtime.sendMessage({ callId: callId, result: result });
                        return [3, 4];
                    case 3:
                        error_1 = _b.sent();
                        chrome.runtime.sendMessage({ callId: callId, error: serialize_error_1.serializeError(error_1) });
                        logger.debug("Error function", functionName, "with", error_1);
                        return [3, 4];
                    case 4: return [3, 6];
                    case 5:
                        if (callId in promiseMap) {
                            _a = request, result = _a.result, error = _a.error;
                            logger.debug("Receive return", callId, result, error);
                            if (error !== undefined) {
                                promiseMap[callId].reject(serialize_error_1.deserializeError(error));
                            }
                            else {
